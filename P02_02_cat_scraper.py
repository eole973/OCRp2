...
if response.status_code == 200:
    ...
    for j in range(3, 53):
        ...
        if link.status_code == 200:
            soup_cat = BeautifulSoup(link.text, 'html.parser')
            ...
            for l in range(2, 7):
                ...
                if answer_cat.status_code == 200:
                    soup_book_page = BeautifulSoup(answer_cat.text, 'html.parser')
                    for r in range(54, 94,2):
                        ...
                        if answer_cat_book.status_code == 200:
                            soup_cat_book = BeautifulSoup(answer_cat_book.text, 'html.parser')
                            # extract item requested
